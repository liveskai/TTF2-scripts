...
function BatteryCharger_Stop( weapon )
{
	...
	OnThreadEnd(
		function() : ( weapon, e )
		{
			if ( IsValid( weapon ) )
			{
				...
				{
					printt( "stop beem effect in end thread", e.handle )
					EffectStop( e.handle, true, false )
					thread HACK_EffectStopNextFrame( expect int( e.handle ), true, false )
				}
			#endif
		}
	)
		...
		//beamTarget.hitPos
			...
			if ( Time() > startTime + 0.05 )
			{
				...
				if ( IsValid( beamTarget.target ) )
				{
					...
					if ( GetEditorClass ( beamTarget.target ) == "toy_arc_switch" )
						damageAmount = 3
					else if ( IsSpectre( beamTarget.target ) || IsStalker( beamTarget.target ) || IsGrunt( beamTarget.target ) )
						damageAmount = 6

					if ( damageAmount > 0 )
					{
						entity owner = weapon.GetWeaponOwner()
						beamTarget.target.TakeDamage( damageAmount, owner, owner, { origin = owner.GetOrigin(), force = <0,0,0>, scriptType = DF_INSTANT | DF_ELECTRICAL | DF_DISSOLVE, weapon = weapon, damageSourceId = eDamageSourceId[ CHARGE_TOOL ] } )
					}
				}
			}
...
BeamTarget function GetBeamTarget( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	if ( weapon.SmartAmmo_IsEnabled() )
	{
		...
		foreach( target in targets )
		{
			...
			smartTarget.target = expect entity( target.ent )
			smartTarget.hitPos = expect vector( target.ent.GetWorldSpaceCenter() )
	...
	else
	{
		...
		foreach( result in results )
		{
			...
			string classname = IsServer() ? visibleEnt.GetClassName() : string(visibleEnt.GetSignifierName())
		...
		//#####################################
		// Get best arc connector toy
		//#####################################

		float angleToAxisCos = deg_cos( angleToAxis )
		BeamTarget beamTargetArcConnector
		array<entity> arcConnectors = GetAllArcConnectors()
		float nearestDist = MAX_BEAM_DISTANCE
		foreach( entity target in arcConnectors )
		{
			if ( !IsValid( target ) )
				continue

			vector entPos = target.GetWorldSpaceCenter()
			float d = Distance( weaponPos, entPos )
			if ( d > MAX_BEAM_DISTANCE )
				continue

			vector vecToEnt = Normalize( entPos - weaponPos )
			float dotVal = DotProduct( vecToEnt, weaponVec )

			#if CLIENT
				if ( DEBUG_DRAW )
				{
					if ( dotVal >= angleToAxisCos )
						DebugDrawSphere( entPos, 8, 0, 255, 0, true, 0.1 )
					else
						DebugDrawSphere( entPos, 8, 200, 200, 200, true, 0.1 )
				}
			#endif
			...
			TraceResults traceResults = TraceLineHighDetail( weaponPos, entPos, weapon, (TRACE_MASK_SHOT | TRACE_MASK_BLOCKLOS), TRACE_COLLISION_GROUP_NONE )
			if (!( traceResults.hitEnt == target || traceResults.fraction > 0.91 ))
				continue
			...
			nearestDist = d
			beamTargetArcConnector.target = target
			beamTargetArcConnector.hitPos = traceResults.endPos//expect vector(entPos)
			beamTargetArcConnector.effectToPlay = CHARGE_BEAM_EFFECT_ENT
		}

		// If we hit both a cone check target and an arc connector target return the closer one
		if ( IsValid( beamTargetArcConnector.target ) && IsValid( beamTargetConeCheck.target ) )
		{
			return DistanceSqr( weaponPos, beamTargetArcConnector.target.GetOrigin() ) < DistanceSqr( weaponPos, beamTargetConeCheck.target.GetOrigin() ) ? beamTargetArcConnector : beamTargetConeCheck
		}

		// If we hit a cone check target, or an arc connector target return the valid one
		if ( IsValid( beamTargetArcConnector.target ) || IsValid( beamTargetConeCheck.target ) )
		{
			if ( IsValid( beamTargetConeCheck.target ) )
				return beamTargetConeCheck
			if ( IsValid( beamTargetArcConnector.target ) )
				return beamTargetArcConnector
		}
	}
...
entity function CreateServerBeamWithControlPoint( entity weapon, asset effect, var controlPoint )
{
	...
	beamSystem.SetParent( weapon.GetWeaponOwner().GetActiveWeapon(), "muzzle_flash", false, 0.0 )
