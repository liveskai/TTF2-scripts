...
global function ServerCallback_ModelViewerDisableConflicts
...
struct {
	...
	vector colorSelected = Vector( 160, 160, 0 )
	vector colorUnselected = Vector( 60, 0, 0 )
	vector colorHover = Vector( 255, 0, 0 )
	vector colorSelectedHover = Vector( 255, 255, 0 )
...
function ClModelViewInit()
{
	RegisterSignal( "NewHLMVPreviewModel" )
	RegisterSignal( "NewHLMVSequence" )
}

function ServerCallback_ModelViewerDisableConflicts()
{
}

function CreateControllerHud()
{
...
function ServerCallback_MVUpdateModelBounds( index, minX, minY, minZ, maxX, maxY, maxZ )
{
	local tab = { mins = Vector( minX, minY, minZ ), maxs = Vector( maxX, maxY, maxZ ) }
	...
	else
	{
		...
		{
			file.modelBounds.append( { mins = Vector( 32.0, 32.0, 32.0 ), maxs = Vector( 32.0, 32.0, 32.0 ) } )
		}
...
function ServerCallback_MVEnable()
{
...
function ServerCallback_MVDisable()
{
...
function ReloadShared()
{
	...
	foreach ( index, elem in file.hudModelNames )
	{
		...
		if ( index < modelViewerModels.len() )
			modelPath = split( modelViewerModels[ index ].tostring(), "/." )
	...
	for ( int i = file.spawnedModels.len() - 1; i >= 0; i-- )
	{
		...
		if ( modelViewerModels.find( model.GetModelName().tolower() ) < 0 )
		{
...
function ControlsButtonYPressed( player )
{
	...
	if ( file.modelToDropIndex >= modelViewerModels.len() )
		file.modelToDropIndex = 0
...
function TumbleModel( player )
{
	...
	while ( file.tumbleModeActive && file.selectedModels.len() > 0 )
	{
		foreach ( model in file.selectedModels )
		{
			local xAxis = InputGetAxis( 0 )
			local yAxis = InputGetAxis( 1 )

			xAxis *= 0.1
			yAxis *= 0.1

			local angles = model.GetAngles()
			...
			if ( fabs( yAxis ) > fabs( xAxis ) )
				angles = AnglesCompose( angles, Vector( yAxis * 10.0, 0.0, 0.0 ) )
			else
				angles = AnglesCompose( angles, Vector( 0.0, xAxis * 15.0, 0.0 ) )

			model.SetAngles( angles )

			local forward = AnglesToForward( AnglesCompose( angles, model.s.centerDiffAngles ) ) * Length( model.s.centerDiff )
	...
	if ( file.selectedModels.len() > 0 )
	{
		foreach ( model in file.selectedModels )
		{
			...
			model.SetAngles( Vector( 0.0, 0.0, 0.0 ) )
...
function DropModelStartTrace()
{
	if ( modelViewerModels.len() <= 0 )
		return

	entity player = GetLocalViewPlayer()
	local playerEyePosition = null

	local traceStartPos = null
	local traceEndPos = null
	...
	while ( file.dpadDownPressed )
	{
		...
		DebugDrawCircle( file.dropModelTraceResult.endPos, Vector( 0.0, 0.0, 0.0 ), 16.0, 255, 0, 0, true, 0.02 )
...
function DropModel()
{
	...
	entity model = CreateClientSidePropDynamic( file.dropModelTraceResult.endPos, Vector( 0.0, 0.0, 0.0 ), modelViewerModels[ file.modelToDropIndex ] )
...
function InitModel( model )
{
	...
	model.s.tumbling <- false
	model.s.center <- Vector( 0.0, 0.0, 0.0 )
	model.s.centerDiff <- Vector( 0.0, 0.0, 0.0 )
	model.s.centerDiffAngles <- Vector( 0.0, 0.0, 0.0 )
	model.s.startOrg <- Vector( 0.0, 0.0, 0.0 )
}

function ModelViewerSpawnModel( pos, ang, idx )
{
	if ( idx >= modelViewerModels.len() )
		return

	file.spawnedModels.append( CreateClientSidePropDynamic( pos, ang, modelViewerModels[ idx ] ) )
...
function SelectModel( entity model )
{
	if ( model == null )
		return
	...
	file.modelToDropIndex = expect int( modelViewerModels.find( model.GetModelName().tolower() ) )

	file.hudGrpModelNames.SetColor( 255, 255, 255, 255 )
	file.hudModelNames[ file.modelToDropIndex ].SetColor( 255, 255, 128 )
}

function TrySelectAllModels()
{
	local delay = Time() + file.selectAllDelay
...
function SwapSelectedModel()
{
	...
	local modelName = file.selectedModels[ 0 ].GetModelName().tolower()
	int currentIndex = expect int( modelViewerModels.find( modelName ) )

	local newIndex = currentIndex + 1

	if ( newIndex >= modelViewerModels.len() )
		newIndex = 0

	file.selectedModels[ 0 ].SetModel( modelViewerModels[ newIndex ] )
...
{
	int modelIndex = expect int( modelViewerModels.find( model.GetModelName().tolower() ) )
	if ( modelIndex < 0 || modelIndex >= file.modelBounds.len() )
		return model.GetOrigin()

	local mins = file.modelBounds[ modelIndex ].mins
	local maxs = file.modelBounds[ modelIndex ].maxs

	return model.GetOrigin() + Vector( mins.x + ( ( maxs.x - mins.x ) * 0.5 ), mins.y + ( ( maxs.y - mins.y ) * 0.5 ), mins.z + ( ( maxs.z - mins.z ) * 0.5 ) )
}

function ShowModelSectionBounds()
{
	if ( file.tumbleModeActive )
		return
	...
	while ( Time() < file.modelSelectionBoundsHideTime )
	{
		...
		foreach ( model in file.spawnedModels )
		{
			local diff = Normalize( GetModelCenter( model ) - player.EyePosition() )

			local dot = diff.Dot( player.GetViewVector() )
		...
		foreach ( model in file.spawnedModels )
		{
			...
			{
				int modelIndex = expect int( modelViewerModels.find( model.GetModelName().tolower() ) )
				vector mins = ( modelIndex >= file.modelBounds.len() || modelIndex < 0 ) ? Vector( -32.0, -32.0, -32.0 ) : expect vector( file.modelBounds[ modelIndex ].mins )
				vector maxs = ( modelIndex >= file.modelBounds.len() || modelIndex < 0 ) ? Vector( 32.0, 32.0, 32.0 ) : expect vector( file.modelBounds[ modelIndex ].maxs )
...
function TranslateModelUp()
{
	...
	while ( file.dpadUpPressed && IsValid( player ) )
	{
		foreach ( model in file.selectedModels )
		{
			if ( IsValid( model ) )
			{
				local trans = model.GetOrigin() - player.GetOrigin()
				trans.z = 0.0
...
function TranslateModelLeft()
{
	...
	while ( file.dpadLeftPressed && IsValid( player ) )
	{
		foreach ( model in file.selectedModels )
		{
			if ( IsValid( model ) )
			{
				local trans = AnglesToRight( Vector( 0, player.CameraAngles().y, 0 ) )
				trans *= -file.modelTranslateSpeed
...
function TranslateModelZUp()
{
	...
	while ( file.triggerRightPressed )
	{
		foreach ( model in file.selectedModels )
		{
			if ( IsValid( model ) )
			{
				local trans = Vector( 0.0, 0.0, 1.0 )
...
function RotateModelCCW()
{
	...
	while ( file.shoulderLeftPressed )
	{
		foreach ( model in file.selectedModels )
		{
			if ( IsValid( model ) )
			{
				model.SetAngles( AnglesCompose( model.GetAngles(), Vector( 0.0, file.modelRotateSpeed, 0.0 ) ) )
			}
		}
		wait( 0.0 )
	}
...
{
	local bounds = { mins = Vector( 0.0, 0.0, 0.0 ), maxs = Vector( 0.0, 0.0, 0.0 ) }
...
function DropModelsToGround()
{
	...
	foreach ( model in file.selectedModels )
	{
		local bounds = GetBounds( model )

		local traceOrg = model.GetOrigin() + Vector( 0.0, 0.0, bounds.mins.z )
		local traceEndPos = traceOrg + Vector( 0.0, 0.0, -16000.0 )

		TraceResults trace = TraceLine( traceOrg, traceEndPos, null, TRACE_MASK_BLOCKLOS, TRACE_COLLISION_GROUP_NONE )

		if ( trace.fraction < 1.0 )
		{
			model.SetOrigin( trace.endPos - Vector( 0.0, 0.0, bounds.mins.z ) )
		}
	}
}
...
function SnapViewToModel( model )
{
	local modelOrg = model.GetOrigin()
	local modelCenter = GetModelCenter( model )
	local modelBounds = GetBounds( model )
	local modelBiggestDimension = max( max( modelBounds.maxs.x - modelBounds.mins.x, modelBounds.maxs.y - modelBounds.mins.y ), modelBounds.maxs.z - modelBounds.mins.z )

	local modelViewDist = modelBiggestDimension * 1.3
	local diagFac = 0.707107

	local viewOffsets = []
	viewOffsets.append( Vector( modelViewDist, 0.0, 0.0 ) )
	viewOffsets.append( Vector( -modelViewDist, 0.0, 0.0 ) )
	viewOffsets.append( Vector( 0.0, modelViewDist, 0.0 ) )
	viewOffsets.append( Vector( 0.0, -modelViewDist, 0.0 ) )
	viewOffsets.append( Vector( modelViewDist * diagFac, modelViewDist * diagFac, 0.0 ) )
	viewOffsets.append( Vector( -modelViewDist * diagFac, -modelViewDist * diagFac, 0.0 ) )
	viewOffsets.append( Vector( modelViewDist * diagFac, -modelViewDist * diagFac, 0.0 ) )
	viewOffsets.append( Vector( -modelViewDist * diagFac, modelViewDist * diagFac, 0.0 ) )

	local playerPos = modelOrg + viewOffsets[ 0 ]
	local viewAng = Vector( 0.0, 0.0, 0.0 )
	local playerEyePos = Vector( 0.0, 0.0, 0.0 )
	...
	foreach ( i, offset in viewOffsets )
	{
		...
		playerEyePos = playerPos + Vector( 0.0, 0.0, 60.0 )

		// position down to ground
		TraceResults trace = TraceLineHighDetail( playerPos, playerPos - Vector( 0.0, 0.0, 256.0 ), player, TRACE_MASK_SHOT, TRACE_COLLISION_GROUP_NONE )
		scores[ i ] = ( 1 - trace.fraction ) * 2.0

		// eye to model origin
		trace = TraceLineHighDetail( playerEyePos, modelOrg + Vector( 0.0, 0.0, 16.0 ), player, TRACE_MASK_SHOT, TRACE_COLLISION_GROUP_NONE )
...
void function ClientCodeCallback_HLMV_ModelChanged_Thread( asset modelName )
{
	...
	if ( !( modelName in file.hlmv_modelData ) )
	{
		...
		data.model = CreateClientSidePropDynamic( Vector(0,0,0), Vector(0,0,0), modelName )
		data.parentEnt = CreateClientSidePropDynamic( Vector(0,0,0), Vector(0,0,0), $"models/dev/empty_model.mdl" )
...
void function DisplayPreviewModelAtCursor( HlmvModelData data )
{
	...
	for ( ;; )
	{
		...
		previewAngles.x = 0
		previewAngles.z = 0
//		previewAngles = AnglesCompose( previewAngles, Vector( 0, 180, 0 ) )
...
void function ClientCodeCallback_HLMV_SequenceChanged_Thread( HlmvModelData data )
{
	...
	{
		CodeWarning( "Can't animate without ref attachment: " + model.GetModelName() )
		return
	}

//	var animStartPos = model.Anim_GetStartForRefPoint_Old( sequence, Vector(0,0,0), Vector(0,0,0) )
	...
	for ( ;; )
	{
		...
		model.SetPlaybackRate( file.hlmv_speedScale );
...
void function ClientCodeCallback_HLMV_SetSpeedScale( float speedScale )
{
	...
	{
		data.model.SetPlaybackRate( speedScale );
...
void function ClientCodeCallback_HLMV_SetCycle( float cycle )
{
	...
	if ( IsValid( data.model ) )
	{
		data.model.SetPlaybackRate( 0.0 );
