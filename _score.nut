...
void function ScoreEvent_TitanKilled( entity victim, entity attacker, var damageInfo )
{
	...
	foreach( DamageHistoryStruct attackerInfo in victim.e.recentDamageHistory )
	{
		...
		if( attackerInfo.attacker != attacker && !exists )
		{
			...
			if ( attackerInfo.attacker.IsTitan() )
			{
				if( victim.GetBossPlayer() || victim.IsPlayer() ) // to confirm this is a pet titan or player titan
					AddPlayerScore( attackerInfo.attacker, "TitanKillTitan", attackerInfo.attacker ) // this will show the "Titan Kill" callsign event
				else
					AddPlayerScore( attackerInfo.attacker, "TitanKillTitan" )
			}
			else
			{
				if( victim.GetBossPlayer() || victim.IsPlayer() )
					AddPlayerScore( attackerInfo.attacker, "KillTitan", attackerInfo.attacker )
				else
					AddPlayerScore( attackerInfo.attacker, "KillTitan" )
			}
